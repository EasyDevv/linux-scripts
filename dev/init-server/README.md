...
상태 파일: `~/.local/share/scripts/dev/init-server/.log/TARGET/state.json`
...
- `init-vps.py` — 원패스 오케스트레이터
- `ovh-ssh-source.py` — OVH Edge Firewall SSH /32
- `host/` — 이 PC에서 도는 Python CLI
- `guest/` — VPS에 복사되는 bash (`source`·systemd ExecStart)
- `early-warning/` — CrowdSec/Falco/Discord 스택
- `profiles/` — 로컬 프로파일 (`*.example.json`만 커밋)
- `tests/` — unittest
