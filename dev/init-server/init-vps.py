...
_netbird_setup = load_py(NETBIRD_DIR / "netbird-setup.py", "netbird_setup")
AppError = _netbird_setup.AppError
atomic_write = _netbird_setup.atomic_write
confirm_no = _netbird_setup.confirm_no
load_config = _netbird_setup.load_config
paint = _netbird_setup.paint
upsert_env = _netbird_setup.upsert_env
BOLD = _netbird_setup.BOLD
CYAN = _netbird_setup.CYAN
DIM = _netbird_setup.DIM
GREEN = _netbird_setup.GREEN
RED = _netbird_setup.RED


_ssh_control = load_py(HOST_DIR / "ssh-control.py", "ssh_control")
ssh_control_options = _ssh_control.control_options
_final_report = load_py(HOST_DIR / "final-report.py", "final_report")
probe_request = _final_report.probe_request
render_report = _final_report.render_report
...
class Runner:
    ...
    def stage_repair_known_hosts(self) -> None:
        if self.after_wipe("repair-hosts.py"):
            return
        if not self.apply:
            self.plan("repair-hosts.py --apply")
            return
        probe = self.probe_ssh()
    ...
    def stage_public_ssh(self) -> None:
        if self.after_wipe("public-ssh.py"):
            return
        argv = [str(HOST_DIR / "public-ssh.py"), self.profile.target]
        if not self.apply:
            self.plan("public-ssh.py")
            return
    ...
    def stage_audit(self) -> None:
        if self.after_wipe("audit-host.py"):
            return
        argv = [str(HOST_DIR / "audit-host.py"), self.profile.target]
        if self.apply:
            self.run_cmd(argv)
        else:
            self.plan("audit-host.py")
    ...
    def stage_bootstrap(self) -> None:
        ...
        self.run_cmd(
            [
                ...
                "-q",
                *self.ssh_control_options,
                str(GUEST_DIR / "bootstrap-debian.sh"),
                str(GUEST_DIR / "setup-log.sh"),
        ...
        run_parallel(
            [
                (
                    "public-ssh",
                    lambda: self.run_cmd(
                        [str(HOST_DIR / "public-ssh.py"), self.profile.target]
                    ),
                ),
                (
                    "setup-log",
                    lambda: self.run_cmd(
                        [str(HOST_DIR / "sync-log.py"), self.profile.target]
                    ),
                ),
            ]
        )
    ...
    def stage_control_plane(self) -> None:
        if self.after_wipe("install-netbird.sh"):
            return
        if self.remote_ok(
            ...
            return
        if not self.apply:
            self.plan("install-netbird.sh --apply")
            return
                ...
                (
                    ...
                    lambda: self.run_cmd(
                        [
                            ...
                            "-q",
                            *self.ssh_control_options,
                            str(GUEST_DIR / "install-netbird.sh"),
                            str(GUEST_DIR / "setup-log.sh"),
                            str(GUEST_DIR / "configure-netbird-setup-guard.sh"),
                            str(GUEST_DIR / "netbird-setup-guard.sh"),
                            f"{self.profile.target}:/tmp/",
                        ]
                    ),
                ),
        ...
        self.ssh_long(
            "sudo bash /tmp/install-netbird.sh --apply "
            f"{self.profile.domain} --setup-source-ip {setup_ip}",
            spinner="install NetBird/Podman",
            log_name="control-plane.out",
        )
        self.run_cmd([str(HOST_DIR / "sync-log.py"), self.profile.target])
        self.state.did_control_plane = True
        save_state(self.state)

    def stage_verify_control_plane(self) -> None:
        if self.after_wipe("verify-netbird.py"):
            return
        argv = [
            str(HOST_DIR / "verify-netbird.py"),
            self.profile.target,
            self.profile.domain,
        ]
        if not self.apply:
            self.plan("verify-netbird.py")
            return
        ...
        with Spinner("waiting for ACME / verify-netbird"):
            while time.time() < deadline:
                ...
                raise Fail("verify-netbird.py failed after waiting for ACME")
    ...
    def stage_host_join(self) -> None:
        if self.after_wipe("host-reconcile.py"):
            return
        ...
        argv = [
            sys.executable,
            str(NETBIRD_DIR / "host-reconcile.py"),
            "--target",
            self.profile.target,
            "--host-selector",
    ...
    def stage_employee_keys(self) -> None:
        ...
        argv = [
            sys.executable,
            str(NETBIRD_DIR / "netbird-setup.py"),
            "employees",
            "--roster",
    ...
    def run_netbird_reconcile(
        ...
        if result.returncode != 0:
            ...
            raise Fail((fail_line or "host-reconcile.py failed") + hint)
        if not save_overlay:
            return
        overlay = ""
    ...
    def stage_early_warning(self) -> None:
        if self.after_wipe("early-warning/apply.py"):
            return
        ...
        if not self.apply:
            ...
            self.plan("early-warning/apply.py")
            return
        self.run_cmd(
            [
                sys.executable,
        ...
        self.run_cmd(
            [str(SCRIPT_DIR / "early-warning" / "apply.py"), self.profile.target],
            spinner="early-warning apply",
        )

    def stage_overlay_alias(self) -> None:
        if self.after_wipe("ssh-alias.py"):
            return
        overlay = self.state.overlay_ip
        if not overlay and not self.apply:
            self.plan("read wt0 and ssh-alias.py")
            return
        if not overlay:
            result = self.ssh(
        ...
        if self.apply:
            ...
            self.plan(f"ssh-alias.py {self.profile.overlay_alias} {overlay}")
        self.state.overlay_ip = overlay
        save_state(self.state)
    ...
    def stage_final(self) -> None:
        ...
        if not self.apply:
            self.plan("public-ssh + verify-netbird + early-warning")
    ...
    def write_completion_report(self, path: Path) -> Path:
        ...
        if not isinstance(facts, dict):
            raise Fail("report-facts.py produced no JSON")
        text = render_report(
            {
