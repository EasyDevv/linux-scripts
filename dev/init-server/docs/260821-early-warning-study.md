...
따라서 저장소 파일을 바꿨다고 서버가 즉시 바뀌는 것은 아니다. `apply.py` 또는 설치 과정을 거쳐야 한다.
...
severity=notice    outcome=ban-or-decision  <- CrowdSec이 차단에 성공
...
- ERROR 이상과 즉시 알림 목록(성공 로그인, CrowdSec decision 등)은 비중요 알림 quota 때문에 막히지 않는다.
...
색은 severity를 따른다. Detector 필드는 CrowdSec, Falco, sshd, auditd처럼 누가 감지했는지 고정 위치에 보여 준다. 제목 아이콘은 event를 빠르게 찾도록 돕는다.
...
1. 로그 본문의 mention은 허용하지 않는다. ERROR 이상은 설정된 사용자만 명시적으로 ping한다.
2. 본문 길이를 제한한다.
3. 본문을 코드 블록으로 묶는다.
...
```bash
# 알림 정책과 embed 구조
python3 -m unittest tests.test_alert_policy -v

# 전체 Python 테스트
python3 -m unittest discover -s tests -p 'test_*.py'

# 셸 문법
bash -n early-warning/install.sh early-warning/verify.sh
```

서버에 적용하는 명령은 profile을 사용한다.

```bash
python3 early-warning/apply.py ovh-vps
```
...
- 사건을 설명하는 제목
- host, detector, event, severity, outcome 필드
- 색이 있는 embed
...
많은 로그
   |
   v
분류(event) + 감지기(detector) + 표준 심각도(severity) + 결과(outcome)
   |
   v
중앙 정책(vps-alert)
   |
