...
class AlertPolicyTests(unittest.TestCase):
    def test_routine_events_are_dropped_but_successful_login_is_immediate(self) -> None:
        ...
        self.assertTrue(alert.should_notify("crowdsec-decision", "notice"))
    ...
    def test_critical_alerts_are_not_blocked_by_noncritical_quota(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            try:
                ...
                self.assertTrue(alert.allow("ssh-login:debian:203.0.113.10", "ssh-login", "info"))
                self.assertTrue(
                    alert.allow("crowdsec-decision:66.132.195.44", "crowdsec-decision", "notice")
                )
            finally:
    ...
    def test_discord_payload_is_a_readable_embed(self) -> None:
        ...
        self.assertEqual(field_values["Detector"], "`sshd`")
    ...
    def test_error_alerts_mention_a_configured_user(self) -> None:
        cfg = {"DISCORD_MENTION": "123456789012345678"}
        content, mentions = alert.ping_mention(cfg, "error")
        self.assertEqual(content, "<@123456789012345678>")
        self.assertEqual(mentions, {"parse": [], "users": ["123456789012345678"]})
        self.assertEqual(alert.ping_mention(cfg, "notice"), ("", {"parse": []}))
        self.assertEqual(alert.ping_mention(cfg, "info"), ("", {"parse": []}))
        self.assertEqual(
            alert.ping_mention({"DISCORD_MENTION": "@everyone"}, "critical"),
            ("", {"parse": []}),
        )

    def test_discord_payload_names_the_detector(self) -> None:
        payload = alert.build_discord_payload(
            "crowdsec-decision",
            "notice",
            "ban-or-decision",
            "[crowdsec] crowdsecurity/http-bad-user-agent ban 4h 66.132.195.44",
        )
        embed = payload["embeds"][0]
        self.assertEqual(embed["title"], "🛡️ CrowdSec decision")
        self.assertEqual(embed["color"], alert.SEVERITY_COLORS["notice"])
        field_values = {field["name"]: field["value"] for field in embed["fields"]}
        self.assertEqual(field_values["Detector"], "`crowdsec`")
        self.assertEqual(field_values["Event"], "`crowdsec-decision`")
        self.assertEqual(field_values["Severity"], "`NOTICE`")
        self.assertEqual(field_values["Outcome"], "`BAN-OR-DECISION`")
        falco_fields = {
            field["name"]: field["value"]
            for field in alert.build_discord_payload(
                "falco", "error", "detected", "Write below binary dir"
            )["embeds"][0]["fields"]
        }
        self.assertEqual(falco_fields["Detector"], "`falco`")
