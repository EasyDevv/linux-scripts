...
type Token = { name: string; value: string; normalizedValue: string; scope: Scope; line: number };
...
const corePath = resolve(
	valueFor("--core") ?? resolve(designDir, "templates/_core/layout.css"),
);
...
async function validateFile(projectPath: string, defaultSource: string) {
	const source = await Bun.file(projectPath).text();
	const errors: string[] = [];
	const root = parseBlock(source, ":root", /:root\s*\{/);
	const dark = parseBlock(source, ".dark", /\.dark\s*\{/);
	const theme = parseBlock(source, "@theme inline", /@theme\s+inline\s*\{/);
	...
	const defaultRootNames = new Set(defaultRoot.tokens.map((token) => token.name));
	const defaultThemeNames = new Set(
		defaultTheme.tokens.map((token) => token.name),
	);
	...
	for (const name of defaultRootNames) {
		if (!rootByName.has(name)) errors.push(`:root missing shadcn token ${name}`);
	}
...
async function projectPaths() {
	...
	const templates = resolve(designDir, "templates");
	const paths: string[] = [];
	for (const entry of await readdir(templates, { withFileTypes: true })) {
		if (!entry.isDirectory()) continue;
		const layout = resolve(templates, entry.name, "layout.css");
...
for (const projectPath of await projectPaths()) {
	reports.push(await validateFile(projectPath, defaultSource));
}

const failed = reports.filter((report) => !report.ok);
process.stdout.write(`${JSON.stringify({ core: corePath, reports }, null, 2)}\n`);
